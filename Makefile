...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=terra \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=nexorad \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/nexorad ./cmd/nexorad
endif

build/linux/amd64:
	GOOS=linux GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o "$@/nexorad" ./cmd/nexorad

build/linux/arm64:
	GOOS=linux GOARCH=arm64 go build -mod=readonly $(BUILD_FLAGS) -o "$@/nexorad" ./cmd/nexorad

build/darwin/amd64:
	GOOS=darwin GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o "$@/nexorad" ./cmd/nexorad

build/darwin/arm64:
	GOOS=darwin GOARCH=arm64 go build -mod=readonly $(BUILD_FLAGS) -o "$@/nexorad" ./cmd/nexorad

build/windows/amd64:
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o "$@/nexorad" ./cmd/nexorad
...
build-linux:
	...
	docker cp temp:/usr/local/bin/nexorad $(BUILDDIR)/
	docker rm temp
...
build-release-amd64: go.sum $(BUILDDIR)/
	...
	$(DOCKER) cp core-builder:/usr/local/bin/nexorad $(BUILDDIR)/release/nexorad
	tar -czvf $(BUILDDIR)/release/terra_$(VERSION)_Linux_x86_64.tar.gz -C $(BUILDDIR)/release/ nexorad
	rm $(BUILDDIR)/release/nexorad
...
build-release-arm64: go.sum $(BUILDDIR)/
	...
	$(DOCKER) cp core-builder:/usr/local/bin/nexorad $(BUILDDIR)/release/nexorad
	tar -czvf $(BUILDDIR)/release/terra_$(VERSION)_Linux_arm64.tar.gz -C $(BUILDDIR)/release/ nexorad 
	rm $(BUILDDIR)/release/nexorad
	$(DOCKER) rm -f core-builder

install: go.sum 
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/nexorad
...
draw-deps:
	...
	@goviz -i ./cmd/nexorad -d 2 | dot -Tpng -o dependency-graph.png
